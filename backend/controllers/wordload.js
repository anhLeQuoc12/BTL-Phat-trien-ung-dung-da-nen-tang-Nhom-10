...
async function CreateWorkload(req, res) {
  ...
  if (!groupId || !listId || !content) {
    return res
      .status(400)
      .json({ error: "groupId, listId, and content are required." });
  }

  try {
    // Xác minh groupId hợp lệ
    const group = await UserGroup.findById(groupId).exec();
    if (!group) {
    ...
    if (group.createdBy.toString() !== userId.toString()) {
      return res
        .status(403)
        .json({ error: "Only the group creator can create workloads." });
    }
    ...
    if (!group.ListId.includes(listId)) {
      return res
        .status(400)
        .json({ error: "The provided listId does not belong to this group." });
    }
